...
# --- FUNCIÓN GENERAR HTML DE BOMBO (ESTA ES LA QUE FALTABA) ---
def generar_html_bombo(nombre_bombo, lista_paises):
    ...
    if not lista_paises:
        ...
        for item in lista_paises:
            ...
            img_tag = ""
            if bandera_url:
                img_tag = f"<img src='{bandera_url}' width='16' style='margin-left:auto; display:block;'/>"
            
            html += f"""
            <div class='country-item'>
                <span style='display:inline-block; width:6px; height:12px; background-color:{color}; margin-right:8px; border-radius:2px; flex-shrink:0;'></span>
                <span style='flex:1; overflow:hidden; text-overflow:ellipsis; margin-right:5px;'>{item['pais']}</span>
                {img_tag}
            </div>
            """
    html += "</div>"
...
# Construcción del grid de bombos usando la función helper definida arriba
